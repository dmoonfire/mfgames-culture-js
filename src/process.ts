...
export class Culture {
    ...
    public parseInstant(input: string, formatId?: string): any {
        ...
        var julian = this.calendar.getBigJulian(instant).plus(offset);
...
export class CultureProvider {
    ...
    private combineCalendars(calendars: Array<Calendar>): Calendar {
        ...
        // clone the cycles because we modify it below.
        var calendarData = { ...calendars[0].data };
        calendarData.cycles = calendars[0].data.cycles
            .map(c => <CalendarCycleData> { ...c });
...
function toBig(input: number|string|BigJsLibrary.BigJS): BigJsLibrary.BigJS {
    switch (typeof (input)) {
        ...
        case "string":
            ...
            if (stringInput.indexOf("/")) {
                // Perform this as a symbolic division.
                var parts = stringInput.split("/").map(a => Big(a.trim()));
                bigString = parts[0].div(parts[1]);
            } else {
